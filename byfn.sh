...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/businessorg.altinn.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/publicorg.altinn.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``BusinessOrg`` & ``PublicOrg``) each managing and maintaining two peer nodes.
...
# (``peer0.businessorg.altinn.com`` & ``peer0.publicorg.altinn.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for BusinessOrgMSP   ##########"
  echo "#################################################################"
  ...
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for BusinessOrgMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for PublicOrgMSP   ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/PublicOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg PublicOrgMSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for PublicOrgMSP..."
    exit 1
  fi
  echo
}
